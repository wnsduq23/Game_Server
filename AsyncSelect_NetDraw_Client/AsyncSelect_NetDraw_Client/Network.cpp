...
bool InitialSocket(HWND hWnd)
{
	...
	ret = WSAAsyncSelect(g_Session.sock, hWnd, WM_MY_SOCKET,
...
void HandleReadEvent(HWND hWnd)
{
	...
	while (useSize > 0)
	{
		...
		if (useSize < HEADER_LEN + header.len)//check
			break;
...
void SendUnicast()
{
	...
	int peekRet = g_Session.sendBuf.Peek(buf, sendBufSize); // 실패할 경우를 대비해서 peek만 함 
